...
ChatGPT Codex는 `F:\dev\ak-codex`, Claude Code는 `F:\dev\ak-claude`, Google Antigravity는 `F:\dev\ak-antigravity`를 고정 worktree로 사용한다. 새 작업은 해당 worktree에서 `git fetch` 후 `git switch -c agent/<topic> main`으로 브랜치를 딴다.
