...
- Python 지원 기준은 3.9 이상입니다.
- 런타임 의존성은 `pydantic`, `pyproj`, `requests`입니다.
...
- WGS84 좌표는 `LatLon(lat, lon)` 순서, AirKorea TM 좌표는 `TmPoint(tm_x, tm_y)` 순서로 고정합니다.
...
- `pyairkorea/coords.py`: WGS84/AirKorea TM 값 객체와 좌표 변환
